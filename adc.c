...
void adc_ch_switch(uint8_t channels){
    switch(channels){
        case 1:
            if (ADCON0bits.GO == 0){            // Verificar si hay conversión en proceso
                ADCON0bits.CHS = 0;             // Selección de canal 0
                __delay_us(40);                 // Delay
                ADCON0bits.GO = 1;              // Iniciar conversión
            }
            break;
        case 2:
            if (ADCON0bits.GO == 0){            // Verificar si hay conversión en proceso
                if (ADCON0bits.CHS == 0){       // Revisar si está en canal 0
                    ADCON0bits.CHS = 1;         // Cambiar a canal 1
                    __delay_us(40);             // Delay
                }
                else if (ADCON0bits.CHS == 1){  // Revisar si está en canal 1
        ...
        case 3:
            if (ADCON0bits.GO == 0){            // Verificar si hay conversión en proceso
                ...
                else if (ADCON0bits.CHS == 2){  // Revisar si está en canal 2
                    ADCON0bits.CHS = 0;         // Cambiar a canal 0
                    __delay_us(40);             // Delay
                }
                __delay_us(40);                 // Delay
